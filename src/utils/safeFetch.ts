...
export async function safeFetch(url: string | Request | URL, options?: RequestInit): Promise<Response> {
  ...
  const isAiEndpoint = urlStr.includes('/api/agent') || urlStr.includes('/api/automation') || urlStr.includes('/api/formatting') || urlStr.includes('/api/extract') || urlStr.includes('/api/exam') || urlStr.includes('/api/convert-document');
